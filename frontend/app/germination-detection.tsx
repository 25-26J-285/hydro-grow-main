...
const BOX_COLORS = ['#16a34a', '#0891b2', '#7c3aed', '#d97706', '#dc2626', '#db2777'];

// Stage order for the progress bar (common germination stages)
const STAGE_ORDER = ['seed', 'germination', 'sprout', 'seedling', 'vegetative', 'flowering'];
...
export default function GerminationDetection() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await fetch(`${normalizedUrl}/api/detect-germination`, {
        method: 'POST',
        body: formData,
      });
  ...
  // Get the top detected stage (highest confidence)
  const topStage = result?.detections?.length
    ? result.detections.reduce((a, b) => (a.confidence > b.confidence ? a : b))
    : null;

  // Find the stage index in the ordered list (case-insensitive partial match)
  const stageIndex = topStage
    ? STAGE_ORDER.findIndex(s => topStage.label.toLowerCase().includes(s))
    : -1;
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerSection}>
          ...
          <View style={styles.headerTextWrap}>
            ...
            <Text style={styles.pageSubtitle}>AI-powered growth stage analysis using YOLOv11</Text>
          </View>
        </View>
        ...
        <View style={styles.heroCard}>
          <View style={styles.heroIconWrap}>
            <MaterialCommunityIcons name="seed-outline" size={30} color={Colors.primary} />
          </View>
          <View style={styles.heroTextWrap}>
            <Text style={styles.heroTitle}>Identify growth stages instantly</Text>
            <Text style={styles.heroSubtitle}>
              Upload or capture a plant photo and our YOLO model will identify the germination stage with bounding box overlays.
            </Text>
          </View>
        </View>
            ...
            <View style={styles.imageWrapper} onLayout={handleWrapperLayout}>
              ...
              {result && imageLayout && result.detections.map((det, i) => {
                ...
                return (
                  <View
                    key={i}
                    style={[styles.bbox, { left: scaled.left, top: scaled.top, width: scaled.width, height: scaled.height, borderColor: color }]}
                  >
        ...
        >
          {isSubmitting ? (
            <>
              ...
              <Text style={styles.analyzeButtonText}>Analyzing growth stage...</Text>
            </>
          ) : (
            <>
              <MaterialCommunityIcons name="leaf-circle-outline" size={20} color="#ffffff" />
              <Text style={styles.analyzeButtonText}>Detect Germination Stage</Text>
            </>
          )}
        </TouchableOpacity>
        ...
        {result && (
          <View style={styles.resultCard}>
            <View style={styles.resultHeader}>
              <View>
                <Text style={styles.resultTitle}>
                  {hasDetections ? 'Stage Identified' : 'No Stage Detected'}
                </Text>
                <Text style={styles.resultSubtitle}>YOLOv11 germination analysis</Text>
              </View>
            ...
            {/* Top Stage Summary */}
            {topStage && (
              <View style={styles.stageSummaryCard}>
                <View style={styles.stageSummaryLeft}>
                  <MaterialCommunityIcons name="sprout" size={36} color={Colors.primary} />
                  <View style={{ marginLeft: 14 }}>
                    <Text style={styles.stageNameText}>{topStage.label.toUpperCase()}</Text>
                    <Text style={styles.stageConfText}>{formatPercent(topStage.confidence)} confidence</Text>
                  </View>
                </View>
              </View>
            )}
            ...
            {stageIndex >= 0 && (
              <View style={styles.progressBlock}>
                <Text style={styles.progressBlockTitle}>Growth Progress</Text>
                  ...
                  {STAGE_ORDER.map((stage, idx) => (
                    <View
                      ...
                      style={[
                        ...
                        idx <= stageIndex && styles.progressSegmentActive,
                      ]}
                    />
                  ))}
                </View>
                  ...
                  {STAGE_ORDER.map((stage, idx) => (
                    <Text
                      ...
                      style={[
                        ...
                        idx === stageIndex && styles.progressLabelActive,
                      ]}
                    >
            ...
            {hasDetections ? (
              <View style={styles.blockCard}>
                <Text style={styles.blockTitle}>All Detections</Text>
              ...
              <View style={styles.noStageCard}>
                <MaterialCommunityIcons name="seed-off-outline" size={36} color="#94a3b8" />
                <Text style={styles.noStageTitle}>No stage detected</Text>
                <Text style={styles.noStageSubtitle}>Try a clearer image with the plant visible in frame.</Text>
              </View>
            )}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  liveChipText: { color: '#047857', fontWeight: '700', fontSize: 12 },
  stageSummaryCard: {
    backgroundColor: '#ecfdf5',
    borderRadius: 18,
    padding: 16,
  ...
  detectionColorBar: { width: 5 },
  detectionBody: { flex: 1, padding: 12 },
  detectionTopRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  detectionLabel: { fontSize: 14, fontWeight: '700', color: '#1f2937', letterSpacing: 0.5 },
  detectionConfidence: { fontSize: 15, fontWeight: '800' },
  confidenceTrack: {
    height: 6,
    borderRadius: 999,
    backgroundColor: '#e2e8f0',
  ...
  noStageTitle: { fontSize: 16, fontWeight: '700', color: '#334155', marginTop: 10 },
  noStageSubtitle: { fontSize: 13, color: '#64748b', marginTop: 6, textAlign: 'center' },
