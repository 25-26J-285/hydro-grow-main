...
from app.services.user_service import create_user, get_user_by_email, serialize_user, verify_password
...
async def get_current_user(token: str = Depends(oauth2_scheme)) -> dict:
    ...
    try:
        ...
        raise credential_exception
    user = await get_user_by_email(token_data.email or "")
    if user is None:
        raise credential_exception
    return user
...
async def register(user: UserRegister):
    ...
    created_user = await create_user(user.email, user.password, user.fullname)
    return {
...
async def login(user: UserLogin):
    ...
    access_token = create_access_token(data={"sub": db_user["email"]}, expires_delta=expires)
    return {
